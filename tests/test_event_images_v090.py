...
def test_invalid_image_extension_and_signature_are_rejected(app, client, users):
    ...
    response = client.post(
        "/events/create",
        data=event_data(image=(BytesIO(PNG_IMAGE), "image.gif")),
        content_type="multipart/form-data",
    )
    assert b"Upload a JPEG, PNG, or WebP image" in response.data
    response = client.post(
        "/events/create",
        data=event_data(image=(BytesIO(b"not an image"), "image.png")),
        content_type="multipart/form-data",
    )
...
def test_oversized_image_is_rejected(app, client, users):
    app.config["EVENT_IMAGE_MAX_BYTES"] = 16
    login(client, users[0])
    response = client.post(
        "/events/create",
        data=event_data(image=(BytesIO(PNG_IMAGE), "large.png")),
        content_type="multipart/form-data",
    )
...
def test_organiser_can_replace_image_and_old_file_is_removed(
    ...
    response = client.post(
        ...
        data=event_data(image=(BytesIO(PNG_IMAGE), "replacement.png")),
        content_type="multipart/form-data",
    )
    assert response.status_code == 302
    with app.app_context():
        new_name = db.session.get(Event, event_id).image_path
