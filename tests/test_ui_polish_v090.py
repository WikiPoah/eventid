...
def test_manage_actions_are_clear_and_owner_scoped(
...
def test_shared_navigation_and_key_routes_render(
    client, users, event_factory
):
    event_id = event_factory()
    login(client, users[0])
    for path in [
        "/events", "/my-events", "/manage-events",
        f"/events/{event_id}", "/events/create",
...
def test_public_event_card_has_scannable_details_action(
    client, users, event_factory
):
