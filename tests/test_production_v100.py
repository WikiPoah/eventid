...
def test_legacy_postgres_url_is_normalized(monkeypatch):
    monkeypatch.setenv("DATABASE_URL", "postgres://user:pass@db/eventid")
...
def test_landing_page_uses_accessible_animation_contract(client):
    ...
    assert b"prefers-reduced-motion" in client.get("/static/css/style.css").data
    assert b"IntersectionObserver" in client.get("/static/js/script.js").data


def test_local_artifact_patterns_are_ignored():
    ignore_rules = open(".gitignore", encoding="utf-8").read()
    for pattern in (".env", "instance/", "*.db", "__pycache__/", "*.pyc"):
