...
def test_anonymous_home_never_calculates_or_renders_recommendations(
    ...
    monkeypatch.setattr(
        ...
        unexpected_recommendation_call,
    )
...
def test_public_carousels_exclude_inaccessible_events(
    client,
    event_factory,
):
    now = datetime.now()
...
def test_each_sliding_carousel_has_unique_controls(
    client,
    event_factory,
):
    now = datetime.now()
    ...
    controlled_ids = re.findall(
        r'aria-controls="([^"]+-viewport)"',
        html,
    )
