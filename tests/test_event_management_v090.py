...
def test_organiser_can_edit_event_and_replace_categories(
    ...
    with app.app_context():
        ...
        db.session.flush()
        db.session.add(EventCategory(event_id=event_id, category_id=old.category_id))
        db.session.commit()
...
def test_capacity_cannot_be_reduced_below_attendance(
    ...
    with app.app_context():
        assert db.session.get(Event, event_id).capacity == 10


def test_edit_database_failure_rolls_back(
    app, client, users, event_factory, monkeypatch
):
    ...
    response = client.post(f"/events/{event_id}/edit", data=edit_data())
    assert response.status_code == 200
